...
# Pola payload QR format LAMA: "AFD {Afdeling} - BLOK {Blok} - TPH {Nomor}"
_POLA_QR = re.compile(r"AFD\s*([^\-]+?)\s*-\s*BLOK\s*([^\-]+?)\s*-\s*TPH\s*(.+)", re.IGNORECASE)

# Pola payload QR format BARU: "KEBUN_AFDELING_BLOK_TPH", contoh "SSM_AF01_A8_001".
# Kata paling depan (kode kebun) diabaikan — yang dibaca cuma 3 bagian
# terakhir: Afdeling, Blok, TPH.
_POLA_QR_UNDERSCORE = re.compile(r"^[^_]+_([^_]+)_([^_]+)_([^_]+)$")
...
def urai_payload_qr(teks):
    ...
    # Coba format BARU dulu (underscore, kebun di depan diabaikan)
